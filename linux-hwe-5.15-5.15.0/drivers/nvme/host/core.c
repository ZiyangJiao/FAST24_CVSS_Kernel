...
/* The REMAP interface */
static inline blk_status_t nvme_setup_remap(struct nvme_ns *ns,
					 struct request *req, struct nvme_command *cmnd,
					 enum nvme_opcode op)
{
	struct nvme_ctrl *ctrl = ns->ctrl;
	...
	cmnd->rw.nsid = cpu_to_le32(ns->head->ns_id);
	cmnd->rw.slba = cpu_to_le64(nvme_sect_to_lba(ns, blk_rq_pos(req)));
	cmnd->rw.rsvd2 = cpu_to_le64(nvme_sect_to_lba(ns, req->__source_sector)); // from host general IO request, can be extended in blkdev.h
	// cmnd->rw.apptag = cpu_to_le16(blk_rq_bytes_apptag(req) >> ns->lba_shift) - 1);  // set up srcLength
	cmnd->rw.length = cpu_to_le16((blk_rq_bytes(req) >> ns->lba_shift) - 1);

	if (req_op(req) == REQ_OP_REMAP && ctrl->nr_streams)
		nvme_assign_write_stream(ctrl, req, &control, &dsmgmt);
...
blk_status_t nvme_setup_cmd(struct nvme_ns *ns, struct request *req)
{
	...
	case REQ_OP_ZONE_APPEND:
		...
		break;
	case REQ_OP_REMAP:
		ret = nvme_setup_remap(ns, req, cmd, nvme_cmd_remap);
		break;
	default:
		WARN_ON_ONCE(1);
		return BLK_STS_IOERR;
